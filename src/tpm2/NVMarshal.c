...
STATE_RESET_DATA_Marshal(STATE_RESET_DATA *data, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_AUTH_Marshal(&data->nullProof, buffer, size);
...
#if RADIX_BITS == 64
    written = UINT64_Marshal(&data->allocated, buffer, size);
    written += UINT64_Marshal(&data->size, buffer, size);
    for (i = 0; i < ARRAY_SIZE(data->d); i++) {
        written += UINT64_Marshal(&data->d[i], buffer, size);
    }
#elif RADIX_BITS == 32
    written = UINT32_Marshal(&data->allocated, buffer, size);
    written += UINT32_Marshal(&data->size, buffer, size);
    for (i = 0; i < ARRAY_SIZE(data->d); i++) {
        written += UINT32_Marshal(&data->d[i], buffer, size);
    }
#endif
...
#if RADIX_BITS == 64
    if (rc == TPM_RC_SUCCESS) {
        rc = UINT64_Unmarshal(&data->allocated, buffer, size);
    }
    if (rc == TPM_RC_SUCCESS) {
        rc = UINT64_Unmarshal(&data->size, buffer, size);
    }
    for (i = 0; i < ARRAY_SIZE(data->d) && rc == TPM_RC_SUCCESS; i++) {
        rc = UINT64_Unmarshal(&data->d[i], buffer, size);
    }
#elif RADIX_BITS == 32
    if (rc == TPM_RC_SUCCESS) {
        rc = UINT32_Unmarshal(&data->allocated, buffer, size);
    }
    if (rc == TPM_RC_SUCCESS) {
        rc = UINT32_Unmarshal(&data->size, buffer, size);
    }
    for (i = 0; i < ARRAY_SIZE(data->d) && rc == TPM_RC_SUCCESS; i++) {
        rc = UINT32_Unmarshal(&data->d[i], buffer, size);
    }
#endif
...
SESSION_Marshal(SESSION *data, BYTE **buffer, INT32 *size)
{
    ...
    written += UINT8_Marshal(&data->commandLocality.val, buffer, size);
...
SESSION_Unmarshal(SESSION *data, BYTE **buffer, INT32 *size)
{
    ...
    if (rc == TPM_RC_SUCCESS) {
        rc = UINT8_Unmarshal(&data->commandLocality.val, buffer, size);
    }
    if (rc == TPM_RC_SUCCESS) {
...
PERSISTENT_DATA_SWAP(PERSISTENT_DATA *t, PERSISTENT_DATA *s, int check)
{
    ...
    if (check &&
        ...
        TPMLIB_LogPrintf("%s: version=%d, magic=0x%08x\n",
                          __func__, s->nvHeader.version, s->nvHeader.magic);
    }
