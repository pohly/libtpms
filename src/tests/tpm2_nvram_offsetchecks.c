...
int persistent_data_check_offsets(void)
{
  ...
  CHECK_OFFSET(PERSISTENT_DATA, SPSeed, 352);
  CHECK_OFFSET(PERSISTENT_DATA, PPSeed, 386);
  CHECK_OFFSET(PERSISTENT_DATA, phProof, 420);
  CHECK_OFFSET(PERSISTENT_DATA, shProof, 470);
  CHECK_OFFSET(PERSISTENT_DATA, ehProof, 520);
  CHECK_OFFSET(PERSISTENT_DATA, totalResetCount, 576);
  CHECK_OFFSET(PERSISTENT_DATA, resetCount, 584);
  CHECK_OFFSET(PERSISTENT_DATA, pcrPolicies, 588);
  CHECK_OFFSET(PERSISTENT_DATA, pcrAllocated, 640);
  CHECK_OFFSET(PERSISTENT_DATA, ppList, 664);
  CHECK_OFFSET(PERSISTENT_DATA, failedTries, 680);
  CHECK_OFFSET(PERSISTENT_DATA, recoveryTime, 688);
  CHECK_OFFSET(PERSISTENT_DATA, lockoutRecovery, 692);
  CHECK_OFFSET(PERSISTENT_DATA, lockOutAuthEnabled, 696);
  CHECK_OFFSET(PERSISTENT_DATA, orderlyState, 700);
  CHECK_OFFSET(PERSISTENT_DATA, auditCommands, 702);
  CHECK_OFFSET(PERSISTENT_DATA, auditHashAlg, 716);
  CHECK_OFFSET(PERSISTENT_DATA, auditCounter, 720);
  CHECK_OFFSET(PERSISTENT_DATA, algorithmSet, 728);
  CHECK_OFFSET(PERSISTENT_DATA, firmwareV1, 732);
  CHECK_OFFSET(PERSISTENT_DATA, firmwareV2, 736);
  CHECK_OFFSET(PERSISTENT_DATA, timeEpoch, 740);

  CHECK_SIZE(PERSISTENT_DATA, 744);
...
int state_reset_data_check_offsets(void)
{
  ...
  CHECK_OFFSET(STATE_RESET_DATA, nullSeed, 58);
  CHECK_OFFSET(STATE_RESET_DATA, clearCount, 92);
  CHECK_OFFSET(STATE_RESET_DATA, objectContextID, 96);
  CHECK_OFFSET(STATE_RESET_DATA, contextArray, 104);
  CHECK_OFFSET(STATE_RESET_DATA, contextCounter, 168);
  CHECK_OFFSET(STATE_RESET_DATA, commandAuditDigest, 176);
  CHECK_OFFSET(STATE_RESET_DATA, restartCount, 228);
  CHECK_OFFSET(STATE_RESET_DATA, pcrCounter, 232);
  CHECK_OFFSET(STATE_RESET_DATA, commitCounter, 240);
  CHECK_OFFSET(STATE_RESET_DATA, commitNonce, 248);
  CHECK_OFFSET(STATE_RESET_DATA, commitArray, 298);

  CHECK_SIZE(STATE_RESET_DATA, 320);
